...
const DateHeading = ({ dateData, GMT }) => {
    ...
    const dateForm = (timeStr) => {
        ...
        switch (i18n.language) {
            ...
            case 'en-US':
                return <>
                    {time.getDate()}<strong>{getOrdinalSuffix(time.getDate())}</strong> {formatMonth(time)} {time.getFullYear()}
                </>
            case 'ko-KR':
                return <>
            ...
            case 'sa-IN':
                return <>
                    <strong>{time.getDate()} {transformSa(time.getMonth())}</strong> {time.getFullYear()}
                </>
            default:
                return <>
                    {time.getDate()}<strong>{getOrdinalSuffix(time.getDate())}</strong> {formatMonth(time)} {time.getFullYear()}
