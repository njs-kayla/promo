...
const Tab = ({ status, data, i, handelClick }) => {
    ...
    return (
        <button className={`tournament-tab ${status} ${isMobile ? 'mobile' : ''}`}
            ...
            <strong className="heading">Angpao {i + 1}</strong>
...
const EventGroupA = ({ eventList, info, GMT }) => {
    const { isMobile } = RWD();
        ...
        <div className="tournament-block">
            {!isMobile && <>
                ...
                <div className="tournament-nav">
                    {eventList.map((group, i) => renderTab(group, i))}
                </div>
            </>}
            {eventList.map((group, i) => (
                <Fragment key={i}>
                    {isMobile && renderTab(group, i)}
                    <div className={`tournament-content ${(activeEvent.group == i) ? 'active' : ''} ${(activeEvent.index[0] > i) ? 'disable' : ''}`}>
                        {isMobile && <div className="tournament-item-gmt">GMT+{GMT}</div>}
                        <div className="tournament-item">
                            {group.map((item, index) => (
                                <DateItem key={index} index={index} data={item} active={activeEvent.index[0] == i && activeEvent.index[1] == index} />
                            ))}
                        </div>
                    </div>
                </Fragment>
...
const EventGroupB = ({ eventList, GMT }) => {
    const { isMobile } = RWD();
    const [activeEvent, setActiveEvent] = useState(null);

    const mergedComplateArray = mergedArray(eventList);
    ...
    const init = () => {
        let activeIndex = 0
        mergedComplateArray.map((item, i) => {
    ...
    const date_template = (timeStr) => {
        ...
        return (
            <>
                {month}<span>/</span>{day}<span>/</span>{year}
            </>
        );
    }
    ...
    return (
        <div className="tournament-block typeB">
            {mergedComplateArray.map((item, i) => (
                <div key={i} className={`date-item typeB ${activeEvent == i ? 'active' : ''}`}>
...
export const EventGroup = ({ eventList, info, GMT, type }) => {
    if (type === 'B') {
        return <EventGroupB eventList={eventList} GMT={GMT} />
    }
    return <EventGroupA eventList={eventList} info={info} GMT={GMT} />
}
