        ...
        switch (i18n.language) {
            ...
            case 'vi-VN':
                return <>
                    ...
                    {time.getFullYear()} {formatAMPM(time)}
                </>
            case 'th-TH':
                return <>
                    <strong>{time.getDate()} {formatMonth(time)}</strong>
                    {(time.getFullYear() + 543)} {formatAMPM(time)}
                </>
            case 'en-US':
                return <>
            ...
            default:
                return <>
                    <strong>{time.getDate()}{getOrdinalSuffix(time.getDate())} {formatMonth(time, true)}</strong> {time.getFullYear()} {formatAMPM(time)}
                </>
        ...
        <>
            <h3 className="bn-date">
                <span className='bn-date-time'>
                    {timeForm(startDate)} <span className='triangle'></span> {timeForm(endDate)}
                </span>
                {!isMobile && <span className="gmt">(GMT + 8)</span>}
            </h3>
        </>
    )
}
