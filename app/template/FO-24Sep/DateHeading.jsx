import { transformSa } from '@/service/util';
...
const DateHeading = ({ dateData, GMT }) => {
    ...
    const dateForm = (timeStr) => {
        ...
        switch (i18n.language) {
            ...
            case 'en-US':
                return <>
                    {time.getDate()}<span className='ordinal'>{getOrdinalSuffix(time.getDate())}</span> {formatMonth(time)} {time.getFullYear()}
                </>
            case 'ko-KR':
                return <>
            ...
            case 'sa-IN':
                return <>
                    {time.getDate()} {transformSa(time.getMonth())} {time.getFullYear()}
                </>
            default:
                return <>
                    {time.getDate()}<span className='ordinal'>{getOrdinalSuffix(time.getDate())}</span> {formatMonth(time)} {time.getFullYear()}
